...
class Breadcrumb_Trail {
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			$breadcrumb .= '<ul class="mds-breadcrumbs-list">';
			...
			foreach ( $this->items as $item ) {
				...
				$item_class = 'mds-trail-item';

				if ( 1 === $item_position && 1 < $item_count )
					$item_class .= ' mds-trail-begin';

				elseif ( $item_count === $item_position )
					$item_class .= ' mds-trail-end';

				// Create list item attributes.
				$attributes = 'class="' . $item_class . '"';

				// Build the list item.
				$breadcrumb .= sprintf( '<li %s>%s</li>', $attributes, $item );
			}
